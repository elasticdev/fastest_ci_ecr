...
class LocalDockerCI(object):
    ...
    def _load_webhook(self,fork,**kwargs):
        ...
        fork["orders"].append(self._get_order(**inputargs))

        return loaded_yaml

    def _clone_code(self,fork,loaded_yaml):
    ...
    def _run(self):
        ...
        # clone code
        self._clone_code(fork,loaded_yaml)
        if fork["status"] == "failed": return fork

        # test code if necessary
        if os.environ.get("DOCKER_FILE_TEST"):
            self._test_code(fork)
            if fork["status"] == "failed": return fork

        # build code
        self._build_container(fork)
        if fork["status"] == "failed": return fork
